...
## 2026-06-06 Task: Tool Cupboard single-block functional refactor

- Replaced the old dual registry names with one `minerust:tool_cupboard` block/item; the block no longer carries a constructor tier.
- `ToolCupboardBlockEntity` is now the persistent mutable tier source, defaults new/missing NBT tier data to 1, and exposes `upgradeToTier2()`.
- Normal owner right-click upgrades tier 1 to tier 2 by consuming 4 diamonds and 4 iron ingots, matching the old tier 2 recipe balance; owner sneak-right-click still authorizes the nearest unauthorized player within 5 blocks.
- `ToolCupboardClaimManager.updateClaimTier` checks expanded-radius overlap with the current TC ignored, removes the old saved claim radius, and re-registers the BE data so upkeep and debug lookups read the wider tier 2 radius.
- Resource/data JSON now uses `tool_cupboard`; tier1/tier2 TC recipes, loot tables, blockstates, models, translations, and tier2 texture were removed, while the old tier1 texture was migrated to `tool_cupboard.png`.


## 2026-06-06 Task: TC placement overlap verification fix

- Fixed the single-TC placement ordering bug where `ToolCupboardBlock.setPlacedBy` could register a new claim before `ClaimProtectionEvents.onBlockPlace` rejected overlap.
- `setPlacedBy` now checks `ToolCupboardClaimManager.wouldOverlap(serverLevel, pos, 1)` before owner/tier initialization and before `registerClaim`, so an invalid placement cannot overwrite `ClaimSavedData` entries.
- `ClaimProtectionEvents.onBlockPlace` no longer removes claims during overlap rejection; it cancels if the TC was not registered by `setPlacedBy` or if another claim still overlaps.
- Valid TC placement still registers exactly once through `setPlacedBy`; owner sneak-right-click authorization and normal right-click upgrade flow are unchanged.

## 2026-06-06 Task: next-session handoff

- Created `NEXT_SESSION.md` at the project root with the current state for a new session.
- Critical user correction captured there: there is no visible/player-facing `tier 2` Security Panel anymore; the object is one `Security Panel` / `Panel de Seguridad`, and any internal tier/radius value should be described to players as coverage/range upgrade.
- The current working tree still has uncommitted changes from the single-panel refactor, Security Panel visual adaptation, docs, and resource migration.

## 2026-06-06 Task: Security Panel coverage/range wording decision

- User correction locked in: there is no visible/player-facing "tier 2" Security Panel anymore.
- Current player-facing language must describe one Security Panel / Panel de Seguridad with upgradeable coverage/range, not a second tiered panel.
- The 4 diamonds + 4 iron ingots upgrade cost stays the same; only the wording changes to coverage/range expansion.
- Internal registry ID `minerust:tool_cupboard` and internal persistence/state remain untouched.

## 2026-06-06 Task: Security Panel visual asset and naming

- Source asset: Sketchfab "minecraft TV model" by DPancito (`https://sketchfab.com/3d-models/minecraft-tv-model-6f5987a2c55b419f9d424d32746b0e35`), CC BY 4.0, downloaded as `/home/alvaro/Descargas/minecraft-tv-model.zip`.
- Archive contents: `source/model.gltf` (109KB, 44 meshes / 49 nodes / 1 material / 1 texture) and `textures/gltf_embedded_0.png` (1453 bytes, 128×128 UV atlas).
- The GLTF model was generated by Blockbench 3.8.4 and is too complex (44 meshes) for direct conversion to Minecraft block model JSON. Instead, a hand-built compact 5-element model was created inspired by the TV geometry.
- New block model `models/block/tool_cupboard.json`: 5 elements (body, screen, control_panel, top_cap, bottom_stand) with depth, totaling ~50 lines. Display transforms included for item/inventory rendering.
- New texture `textures/block/tool_cupboard.png`: procedural 16×16 pixel art generated with Python/Pillow, inspired by the TV model's gray/charcoal/brown palette but themed as a gritty security/control panel with dark screen, button details, and red/green status LED.
- Player-facing rename: "Tool Cupboard" → "Security Panel" (EN), "Armario de Herramientas" → "Panel de Seguridad" (ES). Internal registry `minerust:tool_cupboard` and all Java class names preserved.
- Updated files: block model, texture, `en_us.json`, `es_es.json`, player-facing chat strings in `ClaimProtectionEvents.java`, `ToolCupboardBlock.java`, `ProtectionStaffItem.java`, `ClaimDebugStickItem.java`, `README.md`, `CHECKLIST-V1.md`, `CHANGELOG.md`.
- Build verification: `./gradlew build --no-daemon` passed successfully after all changes.

## 2026-06-06 Task: Security Panel wording cleanup

- Replaced the player-facing Security Panel debug output from tier language to coverage radius / chunk-span wording.
- Aligned CHECKLIST and changelog terminology so the docs now describe the single visible panel as coverage/range expansion, not a visible tier 2.
- Internal claim radius logic and registry IDs were left unchanged.

## 2026-06-06 Task: Security Panel current source of truth

- Supersedes older notes in this file that mention visible tier 1/tier 2 panels, 4 diamonds + 4 iron fixed upgrade cost, chunk-based claims, or a hand-built placeholder TV model.
- Current design: one visible `Security Panel` / `Panel de Seguridad` with internal level 1-20 and registry ID `minerust:tool_cupboard`.
- Coverage is block-centered, not chunk-centered. Level 1 covers 10×60×10, level 20 covers 30×60×30. The GUI intentionally displays only horizontal footprint: `10x10` through `30x30`.
- Placement reserves maximum 30×30 horizontal coverage immediately, so nearby panels cannot later collide during upgrades.
- Upgrade cost is proportional to the next level's horizontal area. Inventory sync is forced after upgrade so the menu updates visually without closing.
- Breaking an upgraded Security Panel refunds half of the cumulative diamond/iron upgrade spend.
- TV model source remains the DPancito Sketchfab/Blockbench GLTF, converted/imported as Forge OBJ with original 128×128 texture; this is no longer a hand-built placeholder model.
- `ROADMAP.md` is now the collaborator dashboard and should be updated whenever implementation reveals new QA checks, decisions, or debt.
